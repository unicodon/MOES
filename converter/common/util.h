...
#endif
#define INFO(...) do { _ftprintf(stdout, __VA_ARGS__); } while (0)

namespace Util
{
	class Reader {
	public:
		Reader(const _TCHAR* path)
		{
			if (_tfopen_s(&m_fp, path, _T("rb"))) {
	...
	tstring getBaseDir(_TCHAR* path)
	{
